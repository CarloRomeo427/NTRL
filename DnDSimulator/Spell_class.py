from DnDSimulator.Ifstatement_class import ifstatements
from random import random
from DnDSimulator.Entity_class import * #should be disabled before running
from DnDSimulator.Token_class import *
from numpy import argmax
from DnDSimulator.Dmg_class import dmg
...
class hunters_mark(spell):
    ...
    def change_hunters_mark(self, target):
        """
        Changes the Hunter's Mark to a new target.
        """
        rules = [
            getattr(self.player, 'can_choose_new_hunters_mark', False),  # Ensure the player can change the mark
            self.is_known,  # Ensure the player knows the spell
            getattr(target, 'state', 0) == 1,  # Ensure the target is conscious
            getattr(self.player, 'bonus_action', 0) > 0  # Ensure the player has a bonus action left
        ]
        errors = [
            f"{self.player.name} tried to change a bound Hunter's Mark",
            f"{self.player.name} tried to change a Hunter's Mark without knowing it",
            f"{self.player.name} tried to change to an unconscious target",
            f"{self.player.name} tried to change a Hunter's Mark without having a bonus action"
        ]

        # Check rules and errors
        if not all(rules):
            for rule, error in zip(rules, errors):
                if not rule:
                    self.DM.say(error, True)
            return  # Exit gracefully without crashing

        # Perform the Hunter's Mark change
        self.DM.say(f"{self.player.name} changes the Hunter's Mark to {target.name}", True)
        self.player.bonus_action = 0  # Use a bonus action
        self.player.can_choose_new_hunters_mark = False  # Prevent further changes this turn

        # Add a new Hunter's Mark token to the target
        NewHuntersMarkToken = HuntersMarkedToken(target.TM, subtype='hm')  # Mark the target
        if self.player.CurrentHuntersMarkToken:
            self.player.CurrentHuntersMarkToken.addLink(NewHuntersMarkToken)  # Add the new token
        else:
            self.DM.say(f"{self.player.name} tried to mark {target.name}, but no current Hunter's Mark token was found.", True)
